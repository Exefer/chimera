...
impl Application {
    /// Creates a new instance of `Application` and initializes it with the configuration.
...
#[tauri::command]
fn get_default_configuration() -> config::ChimeraConfig {
    config::ChimeraConfig::default()
}

#[tauri::command]
fn get_current_configuration(state: tauri::State<'_, Application>) -> ApplicationStatus {
    let state = state.shared_state.read();
    match &*state {
        Some(s) => ApplicationStatus {
...
#[tauri::command]
pub fn list_torrents(state: tauri::State<Application>) -> Result<TorrentListResponse, ApiError> {
