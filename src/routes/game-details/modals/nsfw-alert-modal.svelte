...
<Dialog.Root open={hasNSFWContentBlocked}>
  <Dialog.Content interactOutsideBehavior="ignore" hideCloseButton>
    <Dialog.Header>
      <Dialog.Title>{$t("game_details.nsfw_content_alert:title")}</Dialog.Title>
    </Dialog.Header>
    <Separator />
    <Dialog.Description>
      {$t("game_details.nsfw_content_alert:description", {
        values: {
          title,
        },
      })}
    ...
    <Dialog.Footer>
      <Button variant="outline" onclick={onCancel}
        >{$t("game_details.nsfw_content_alert:refuse")}</Button
      >
      <Button variant="destructive" onclick={onConfirm}
        >{$t("game_details.nsfw_content_alert:allow")}</Button
      >
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>
